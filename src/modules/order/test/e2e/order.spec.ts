...
describe('order controller    (e2e)', () => {
  ...
  describe('create Order ', () => {
    it('/ (order/create) valid hot', async () => {
      const baker = await dbConnection.collection('users').insertOne({
        ...bakerEx1,
        password: await hashPassword(bakerEx1.password),
      });
      const cake = await dbConnection
        .collection('cakes')
        .insertOne({ ...cakeEx1, baker: new Types.ObjectId(baker.insertedId) });
  ...
  describe('respond to  Orders ', () => {
    it(' /Order/respondToOrder/:id accept', async () => {
      ...
      const res = await request(httpServer)
        .post(`/order/respondToOrder/${order.insertedId}`)
        .set({
  ...
  describe('collect  order ', () => {
    it('/Order/collectOrder/:id valid', async () => {
      ...
      const order = await dbConnection.collection('orders').insertOne({
        ...OrderEx1,
        cake: { ...cakeEx1, baker: baker.insertedId },
        status: OrderStatus.accepted,
      });

      const bakerToken2 = await testSignToken(
        {
      ...
      const res = await request(httpServer)
        .post(`/order/collectOrder/${order.insertedId}`)
        .set({
  ...
  describe('rate  ', () => {
    it('/Order/rateorder/:id valid', async () => {
      ...
      const res = await request(httpServer)
        .post(`/order/rateorder/${order.insertedId}`)
        .set({
          authorization: `Bearer ${memeberToken}`,
        })
        .send({ rate: 5 });
