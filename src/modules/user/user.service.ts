...
export class UserService extends AbstractService<UserDocument> {
  ...
  async getAvailableCollectingTimes(baker: User, cake: Cake) {
    ...
    const bakerFirstCollectingTime = Number(
      addTime(bakerTimeRange.start.hours, bakerTimeRange.start.miutes, today),
    );
    const bakerLastCollectingTime = Number(
      addTime(bakerTimeRange.end.hours, bakerTimeRange.end.miutes, today),
    );

    const nowPlusBaking = Number(
      addTime(cake.bakingTime.hours, cake.bakingTime.miutes),
    );
    ...
    const availableTimes = getFreePeriods(
      orders,
      startTime,
      bakerLastCollectingTime,
      bakingTime,
    );
...
function getFreePeriods(
  orders: Order[],
  startTime: number,
  bakerLastCollectingTime: number,
  bakingTime: number,
) {
  ...
  orders.forEach((order, index) => {
    ...
    if (periodDiff > bakingTime) {
      period.start =
        period.start + bakingTime + appSettings.waitingTimeforPendingOrders;
      availableTimes.push(period);
    }
  });
